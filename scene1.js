...
(function () {
  ...
  function initQuiz() {
    ...
    document.getElementById("quiz-submit").addEventListener("click", onSubmit);
  }
  ...
  function onSubmit() {
    ...
    // This filtered array is passed to all downstream scenes via STATE.
    ...
    // Also store aggregated stats per car for Scene 5
  ...
  // ── helpers ───────────────────────────────────────────────────────────────
  // Fuzzy trip category match (dataset values may differ from button values)
  ...
  function aggregateByCar(data, priority) {
    // Group by car_name, compute averages
    ...
    // Sort: lower is better for cost + emissions, higher is better for mileage
    arr.sort((a, b) =>
  ...
  function revealSignsSequentially() {
    ...
    signs.forEach((sign, i) => {
      setTimeout(() => sign.classList.add("visible"), 300 + i * 280);
    });
    // show submit placeholder text
    setTimeout(() => {
      document.getElementById("quiz-submit").hidden = false;
      document.getElementById("quiz-submit").disabled = true;
      document.getElementById("quiz-submit").style.opacity = "0.4";
    }, 300 + signs.length * 280);
  }
